...
# Если Бруно в сцене ещё нет — скрипт сам соберёт его билдером (в билдере
# уже все правки v4) и на этом закончит.
import bpy
...
D = bpy.data
_fresh_build = False
if not (D.collections.get("Bruno") and D.objects.get("Bruno_Rig")):
    _code = open(BUILDER).read()
    exec(compile(_code, BUILDER, "exec"), {"__file__": BUILDER, "__name__": "__main__"})
    print("Patch v4: Бруно не было в сцене — собран билдером (уже включает v4).")
    _fresh_build = True
...
def _apply_patch():
    ...
    # (пропускается, если Бруно только что собран билдером)
    removed = 0
...
if _fresh_build:
    print("Patch v4: свежая сборка уже в v4 — точечные правки не нужны.")
else:
    _apply_patch()