...
коллайдера НЕТ — по клумбе можно ходить (Артур).
...
убраны по просьбе Артура). friend_*.glb v=3: yellow Idle_7+Look, red
Idle_9+Look (Idle_15 держит руку вверху), pink Idle_3+Look+Chat(Idle_9).
Stand_and_Chat УБРАН у всех — он разворачивает корпус боком («кривая
стойка»). Айдлы рассинхронизированы (timeScale 0.9–1.1, случайная фаза).
...
- Видео: memory1.mp4 — ОРИГИНАЛ Kling 55МБ (пережатие в 720p Артур
  отклонил: мыло+кроп; encode_video.py остаётся в tools/blender; Blender 5:
  media_type='VIDEO' до file_format='FFMPEG'). object-fit: contain (без
  кропа). preloadVideo(id) качает блоб фоном после загрузки мира — к
  моменту показа ролик уже на диске, не стримится.
...
- SafetyLayer idle-hint: 30с бездействия в EXPLORE, max 3 раза за сессию,
  текст = props.json locked (+ подсветка Бруно), ТОЛЬКО до brunoMet; после
  миссии подсказок нет. Старая зашитая «A little step is enough» убрана.
...
- Озвучка ПОЛНОСТЬЮ закрыта (s7_sophie_look_bruno записан). Хвостов от
  Артура нет. Видео в оверлее: object-fit cover (Артур попросил без полос).
  Фонтан: drinkBusyUntil (5.6с) — тапы во время питья молчат; кулдаун 2 мин
  отдельно. StoryEngine.muteNextPrompt: после redirect/avoidant выбора
  следующий вопрос (тот же или soft_support) не озвучивается.
