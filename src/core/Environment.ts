...
const TILE_CELLS = (() => {
  ...
  addRun(range(-14, 6), [8])         // поперечная: запад -> фонтан -> лавка
  addRun([-6], range(9, 13))         // фонтан -> южный периметр
  addRun([5], range(9, 13))          // лавка -> южный периметр
...
function buildGrass(root: THREE.Group): void {
  ...
  const AVOID_C: [number, number, number][] = [
    [-3.0, -11.6, 1.7], [4.4, -12.6, 2.0], [8.5, -9.4, 2.2],
    [7.3, 8.6, 1.4], [-8.6, -6.4, 1.3], [8.6, 5.4, 0.7],
    [-9.4, -2.0, 1.0], [-5.6, 3.4, 0.9],
    [-5.5, 9.6, 1.9], // фонтан
    [3.3, -8.15, 0.8], [4.9, -9.1, 0.8], [6.5, -8.15, 0.75], // друзья
  ]
  const AVOID_R: [number, number, number, number][] = [
    [6.6, 13.8, -6.2, 0.2], // дом справа (x0,x1,z0,z1)
    [-4.8, 5.5, 10.2, 13.5], // сад-клумба на юге
  ]
...
const COLL_CIRCLES: [number, number, number][] = [
  [-3.0, -11.6, 1.5], [4.4, -12.6, 1.7], [8.5, -9.4, 1.9],
  [7.3, 8.6, 1.1],
  // стволы деревьев (крона не мешает)
  [-8.6, -6.4, 0.6], [-12.8, -12.8, 0.6], [12.9, -12.9, 0.6],
  [-12.6, 11.8, 0.6],
  // кусты
  [-6.2, -15.1, 0.8], [13.9, -7.2, 0.7], [-15.1, -2.0, 0.75],
  [10.4, 13.6, 0.7], [13.3, 9.0, 0.75], [-8.8, 13.9, 0.7],
  [-12.0, -15.1, 0.7], [13.8, 1.4, 0.65],
  // Бруно и друзья (жёлтый/красный/розовый)
  [-3.0, -8.0, 0.9],
  [3.3, -8.15, 0.75], [4.9, -9.1, 0.75], [6.5, -8.15, 0.7],
  // фонтан
  [-5.5, 9.6, 1.5],
]
